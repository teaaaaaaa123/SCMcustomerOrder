...
def load_pattern_rules():
    ...
    if pattern_rules is None:
        rules_file = os.path.join(os.path.dirname(__file__), 'MTM_all_category_rules.json')
...
# 根据版型描述匹配版型编码
def match_pattern_by_description(description):
    """
    根据版型描述（如"经典款平驳头10cm全里"）匹配对应的版型编码
    :param description: 版型描述文本
    :return: 匹配到的版型编码，如果没有匹配到返回None
    """
    ...
    # 提取驳头类型（规则1）
    if '平驳头' in description:
        features.append('平驳头')
    elif '戗驳头' in description:
        features.append('戗驳头')
    elif '青果领' in description:
        features.append('青果领')
    
    # 提取驳头宽度（规则2）- 提取数字
    width_match = re.search(r'(\d+(?:\.\d+)?)\s*cm', description)
    if width_match:
        features.append(width_match.group(1))
    else:
        # 尝试提取纯数字（可能是宽度）
        num_match = re.search(r'\b(\d+(?:\.\d+)?)\b', description)
        if num_match:
            features.append(num_match.group(1))
    
    # 提取纽扣数量（规则3）
    if '二扣' in description:
        features.append('二扣')
    elif '一扣' in description:
        features.append('一扣')
    elif '三扣' in description:
        features.append('三扣')
    elif '双排' in description:
        if '六扣二' in description:
            features.append('双排六扣二')
        elif '四扣一' in description:
            features.append('双排四扣一')
        elif '二扣一' in description:
            features.append('双排二扣一')
        elif '六扣三' in description:
            features.append('双排六扣三')
    
    # 提取里布类型（规则4）
    if '全里' in description:
        features.append('全里')
    elif '半里' in description:
        features.append('半里')
    elif '无里' in description:
        features.append('无里')
    
    # 如果没有提取到足够特征，直接返回None
    if len(features) < 2:
        return None
    ...
    for rule in rules:
        ...
        if rule.get('规则1'):
            rule_feature1 = rule['规则1'].strip().replace('\n', '').replace('（', '').replace('）', '')
            rule_features.append(rule_feature1)
        if rule.get('规则2'):
            rule_feature2 = rule['规则2'].strip().replace('\n', '').replace('（', '').replace('）', '')
            rule_features.append(rule_feature2)
        if rule.get('规则3'):
            rule_feature3 = rule['规则3'].strip().replace('\n', '').replace('（', '').replace('）', '')
            rule_features.append(rule_feature3)
        if rule.get('规则4'):
            rule_feature4 = rule['规则4'].strip().replace('\n', '').replace('（', '').replace('）', '')
            rule_features.append(rule_feature4)
        
        # 计算匹配数量（精确匹配）
        match_count = 0
        has_lapel_match = False  # 标记是否匹配了驳头类型
        
        for feature in features:
            # 检查是否匹配规则1（驳头类型）- 必须匹配
            if rule_feature1 and (feature == rule_feature1 or 
                                  (len(feature) > 2 and feature in rule_feature1)):
                match_count += 1
                has_lapel_match = True
                continue
            # 检查是否匹配规则2（宽度）- 需要精确匹配
            if rule_feature2:
                # 尝试精确匹配数字部分
                feature_num = re.search(r'\d+(?:\.\d+)?', feature)
                rule_num = re.search(r'\d+(?:\.\d+)?', rule_feature2)
                if feature_num and rule_num and feature_num.group() == rule_num.group():
                    match_count += 1
                    continue
            # 检查是否匹配规则3（纽扣）- 需要精确匹配
            if rule_feature3 and feature == rule_feature3:
                match_count += 1
                continue
            # 检查是否匹配规则4（里布）- 需要精确匹配
            if rule_feature4 and feature == rule_feature4:
                match_count += 1
                continue
        
        # 必须匹配驳头类型，并且至少匹配3个特征
        if has_lapel_match and match_count >= 3 and match_count > max_matches:
            max_matches = match_count
            matched_pattern = rule.get('版型编码')
...
API_CONFIG = {
    ...
    # 图片上传API（OSS直传：先获取凭证，再直传到OSS）
    "upload_cred_url": "https://scm.ceyadi.cn/api/admin/base/comm/upload",
    # Credentials are injected by OpenClaw for the current run.
    "access_key_id": "",
    "access_key_secret": "",
    "cached_token": None
}
...
def parse_text_order(text):
    ...
    for group_index, group_text in enumerate(order_groups):
        ...
        if fabric_supply_match:
            fabric_supply = fabric_supply_match.group(1).strip()
        else:
            # 尝试英文格式：PURCHASE BY CUSTOMER / PURCHASE BY CEYADI
            fabric_supply = ""
            if re.search(r'PURCHASE\s+BY\s+CUSTOMER', group_text, re.IGNORECASE):
                fabric_supply = "面料客供"
            elif re.search(r'PURCHASE\s+BY\s+CEYADI', group_text, re.IGNORECASE):
                fabric_supply = "工厂代购"
        # 兜底默认值
        if not fabric_supply:
            fabric_supply = "面料客供"
        ...
        # 先从文本中提取尺码信息（独立于版型编码）
        # 中文格式：48码, 48R码, 48C码
        size_from_text = re.search(r'(\d+[RCrc\-]?)\s*码', group_text)
        if size_from_text:
            size_str = size_from_text.group(1)
            # 分离尺码数字和落差
        ...
        # 如果仍然没有找到版型编码，尝试使用版型描述进行匹配
        if not pattern_matches:
            # 尝试从文本中提取版型描述（支持包含数字、字母、空格）
            description_patterns = [
                r'(经典款[\u4e00-\u9fa5\d\scm]+?)(?=的\s*\d+码|，|。|$)',
                r'(平驳头[\u4e00-\u9fa5\d\scm]+?)(?=的\s*\d+码|，|。|$)',
                r'(戗驳头[\u4e00-\u9fa5\d\scm]+?)(?=的\s*\d+码|，|。|$)',
                r'(青果领[\u4e00-\u9fa5\d\scm]+?)(?=的\s*\d+码|，|。|$)',
                r'(经典款[\u4e00-\u9fa5\d\scm]+)',
                r'(平驳头[\u4e00-\u9fa5\d\scm]+)',
                r'(戗驳头[\u4e00-\u9fa5\d\scm]+)',
                r'(青果领[\u4e00-\u9fa5\d\scm]+)',
            ...
            # 如果没有找到具体描述，尝试使用整个group_text
            if not description:
                description = group_text
            
            # 尝试匹配版型编码
            matched_pattern_code = match_pattern_by_description(description)
            if matched_pattern_code:
                # 使用匹配到的版型编码
                if final_size:
                    pattern_matches.append((matched_pattern_code, final_size + '码'))
                    if final_drop:
                        drop = final_drop
                else:
                    # 如果没有尺码信息，尝试从文本中提取
                    ...
                    if size_from_text:
                        ...
                        if '-' in size_str:
                            ...
                            drop = re.search(r'([RCrc])', size_str).group(1).upper()
                        else:
                            drop = 'R'
                        pattern_matches.append((matched_pattern_code, final_size + '码'))
                    else:
                        pattern_matches.append((matched_pattern_code, '50码'))
                        drop = 'R'
            
            # 如果版型映射找不到，返回错误信息
            if not matched_pattern_code:
                result["items"] = []
                return {
                    "success": False,
                    "message": f"无法根据版型描述 '{description}' 找到对应的版型编码，请提供正确的版型编码或版型描述"
                }
...
def api_upload_image(image_path):
    """
    上传图片到OSS，返回图片URL
    流程：1.获取OSS直传凭证 2.直传文件到OSS 3.拼接URL
    """
    try:
        ...
        # 步骤1：获取 OSS 直传凭证
        cred_url = API_CONFIG.get("upload_cred_url", "https://scm.ceyadi.cn/api/admin/base/comm/upload")
        cred_resp = requests.post(cred_url, headers=headers, timeout=30)
        print(f"获取OSS凭证状态码: {cred_resp.status_code}")

        if cred_resp.status_code != 200:
            return {"success": False, "message": f"获取OSS凭证失败，状态码: {cred_resp.status_code}"}

        cred_result = cred_resp.json()
        if cred_result.get("code") != 1000:
            return {"success": False, "message": f"获取OSS凭证失败: {cred_result.get('message', '未知错误')}"}

        cred_data = cred_result["data"]
        host = cred_data["host"]
        policy = cred_data["policy"]
        oss_access_key_id = cred_data["OSSAccessKeyId"]
        signature = cred_data["signature"]
        success_action_status = str(cred_data.get("success_action_status", 200))

        # 步骤2：构造 OSS 文件 key（格式：app/YYYY-MM-DD/uuid.ext）
        ext = os.path.splitext(image_path)[1].lstrip(".") or "png"
        date_str = datetime.now().strftime("%Y-%m-%d")
        key = f"app/{date_str}/{uuid.uuid4()}.{ext}"

        # 步骤3：直传文件到 OSS（表单提交，file 必须放最后）
        with open(image_path, "rb") as f:
            ...
            files = {"file": f}
            oss_resp = requests.post(host, data=form_data, files=files, timeout=60)

        print(f"OSS直传状态码: {oss_resp.status_code}")

        if oss_resp.status_code != 200:
            return {"success": False, "message": f"OSS直传失败，状态码: {oss_resp.status_code}，响应: {oss_resp.text[:200]}"}
